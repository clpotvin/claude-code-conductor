...
- **Status**: `orchestrate status --project "$(pwd)"`
- **Pause**: `orchestrate pause --project "$(pwd)"` — sends a graceful pause signal. Workers finish their current task, then the orchestrator pauses. Resume later with `orchestrate resume`.
