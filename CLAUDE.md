...
Workers are full Claude Code sessions spawned via the Agent SDK. They coordinate through a custom MCP server with shared contracts, architectural decisions, and dependency context. All run state persists to `.orchestrator/` in the target project directory.
...
- **Flow Tracer** (`src/core/flow-tracer.ts`) — Spawns read-only workers to trace user flows through code changes across all configured layers. Loads project-specific flow config from `.orchestrator/flow-config.json`. Runs in parallel with code review.
...
- **Rules Loader** (`src/utils/rules-loader.ts`) — Loads `.orchestrator/rules.md` or `.orchestrator/worker-rules.md` for project-specific rules injected into worker prompts.
...
- **StateManager** (`src/core/state-manager.ts`) — Persists `OrchestratorState` to `.orchestrator/state.json`. All tasks, sessions, messages, contracts, decisions, and reviews also persist under `.orchestrator/`.
...
- **Flow Config** (`src/utils/flow-config.ts`) — Loads per-project flow-tracing configuration from `.orchestrator/flow-config.json` with generic defaults.
...
- **Configurable per-project**: `.orchestrator/rules.md` for worker rules, `.orchestrator/flow-config.json` for flow-tracing layers/actors/edge-cases.
